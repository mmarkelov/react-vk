    ...
    VK.Api = {
      ...
      checkMethod: function(method, params, cb, queryTry) {
        ...
        if (m == "messages.allowmessagesfromgroup") {
          ...
          query =
            VK._protocol +
            "//vk.com/widget_allow_messages_from_community.php?act=allow_box&group_id=" +
    ...
    _change: function(state, index) {
      ...
      for (var i = 0; i < 2; ++i) {
        ...
        if (state === 0) {
          ...
          elem.style.borderLeftColor = elem.style.borderRightColor = elem.style.borderBottomColor =
            "#5C82AB";
        } else if (state == 1) {
          ...
          elem.style.borderLeftColor = elem.style.borderRightColor = elem.style.borderTopColor =
            "#51779F";
        }
      }
      if (state === 0 || state == 2) {
    ...
    return VK.Widgets._constructor(
      ...
      params,
      {
        showBox: function(url, props) {
          var box = VK.Util.Box(
            VK.Widgets.showBoxUrl(options.base_domain, url),
            [],
            {
              proxy: function() {
  ...
  VK.Widgets.Ads = function(objId, options, paramsExtra) {
    ...
    function adsAttachScript(adsScriptVersion) {
      if (!("vk__adsLight" in window)) {
        ...
        attachScriptFunc(
          base_domain + "/js/al/aes_light.js?" + adsScriptVersion
        );
      } else if (
        window.vk__adsLight &&
      ...
      instances[objId] = VK.Widgets._constructor(
        ...
        params,
        {
          ...
          fatalError: function(error_code, public_id) {
            ...
            CommunityMessages.destroy(objId);
            var box = VK.Util.Box(
              VK.Widgets.showBoxUrl(
                options.base_domain,
                "blank.php?" + query_str.join("&")
              )
            );
            box.show();
          },
